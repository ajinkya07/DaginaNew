        ...
        return (
            <TouchableOpacity onPress={() => this.getProductGridOrNot(item)}>
                <Animatable.View animation="flipInX" style={{ paddingTop: hp(0.5), paddingBottom: hp(0.5) }}>
                    <View style={{ flexDirection: 'row', flex: 1, marginLeft: hp(2), marginRight: hp(1) }}>
                        <View style={{ flex: 0.30, justifyContent: 'flex-start', }}>
                            <Image
                                style={{
                                    height: hp(10), width: hp(12), borderRadius: 10,
                                    borderWidth: 0.4, borderColor: color.gray
                                }}
                                source={{ uri: baseUrl + item.image_name }}
                                defaultSource={require('../../../assets/image/logo.png')}
                            />
                        </View>
